...
export default {
  ...
  createPaste() {
    ...
    if (atom.config.get("atom-pastebin-plus.use_filename")) {
      var title = atom.workspace.getActiveTextEditor().getTitle();
    } else {
      var title = "Untitled";
    }


    this.uploadToPastebin(title, text).then(function(u) {
      var detail = "";
  ...
  uploadToPastebin(title, content) {
